...
This document lists the available REST endpoints and their data structures for the Mission Control
Service.
...
- `prefix`: String - Unique prefix for tasks in the project.
...
- `taskCode`: String - Unique human-readable code for the task (e.g., "PRJ-1"). Not updatable.
...
- **Output**: `ProjectDto` (JSON)
- **Description**: Updates the status of a project. If status is BLOCKED, the `blockedReason` can be
...
- **Output**: `TaskDto` (JSON)
- **Description**: Updates the status of a task. If status is BLOCKED, the `blockedReason` can be
  provided in the request body.
